(function () {
    $(function () {
        ...
        var _$form = $('form[name=withdrawalCreateForm]');
        var _withdrawalService = abp.services.app.withdrawal;
        ...
        var caseprice;
        _$form.validate({});
        var lastWithdrawalCode;
        var newWithdrawalCode;

        let today = new Date().toLocaleDateString();
        $("#withdrawalDate").val(today);
        $("#withdrawalDate").attr('readonly', true);

        _withdrawalService.getLastWithdrawalCode().done(function (result) {
            if (result == null) {
                var one = "V0000001";
                $("#withdrawalNumber").val(one);
            }
            else {
                lastWithdrawalCode = result.withdrawalCode;
                var getPart = lastWithdrawalCode.replace(/[^\d.]/g, ''); //get number part
                var num = parseInt(getPart); //convert to int
                var newval = num + 1; //add 1
                var reg = new RegExp(num); //dynamic regexp
                newWithdrawalCode = lastWithdrawalCode.replace(reg, newval);
                $("#withdrawalNumber").val(newWithdrawalCode);
            }

        });
        for (var i = 0; i < 10; i++) {
            $("#withdrawalTable").append(
                "<tr id='rowProd" + i + "'>" +
        ...
        $("#withdrawalTable").on('click', '.delProd', function () {
            $(this).closest('tr').remove();
            getTotal();
        });

        $("#withdrawalTable").on('change', '#prodCode', function () {
        ...
        $("#withdrawalTable").on('change', '#pieces', function () {
        ...
        _$form.find('button[type="submit"]').click(function (e) {
            ...
            var withdrawal = _$form.serializeFormToObject();
            ...
            $("input[name=ProductId]").each(function () {
                ...
                if ($(this).val() > 0) {
                    ...
                    withdrawal.withdrawalDetails.push(data);
                }
            });
            console.log(withdrawal);
            abp.ajax({
                url: abp.appPath + 'Withdrawals/CreateWithdrawal',
                type: 'POST',
