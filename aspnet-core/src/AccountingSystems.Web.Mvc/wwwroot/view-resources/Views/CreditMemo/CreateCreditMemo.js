(function () {
    $(function () {
        ...
        var _$form = $('form[name=creditMemoCreateForm]');
        ...
        var caseprice;
        _$form.validate({});
        var lastCreditMemogCode;
        var newCreditMemoCode;

        let today = new Date().toLocaleDateString();
        $("#creditMemoDate").val(today);
        ...
        _creditMemoService.getLastCreditMemoCode().done(function (result) {
            if (result == null) {
                var one = "CM0000001";
                $("#creditMemoNumber").val(one);
            }
            lastCreditMemogCode = result.creditMemoCode;
            var getPart = lastCreditMemogCode.replace(/[^\d.]/g, ''); //get number part
            var num = parseInt(getPart); //convert to int
            var newval = num + 1; //add 1
            var reg = new RegExp(num); //dynamic regexp
            newCreditMemoCode = lastCreditMemogCode.replace(reg, newval);
            $("#creditMemoNumber").val(newCreditMemoCode);
        });
        for (var i = 0; i < 10; i++) {
        ...
        _$form.find('button[type="submit"]').click(function (e) {
            ...
            abp.ajax({
                url: abp.appPath + 'CreditMemo/CreateCreditMemo',
                type: 'POST',
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                dataType: 'json',
                data: creditMemo,
                success: function () {
                    location.href = "/CreditMemo";
                },
                error: function (e) { console.log(e); }
            });
            /*abp.ui.setBusy(_$form);
            _creditMemoService.create(creditMemo).done(function () {
                location.href = "/CreditMemo";
            }).always(function () {
