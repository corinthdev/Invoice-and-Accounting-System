(function () {
    $(function () {
        ...
        var _$form = $('form[name=bookingCreateForm]');
        ...
        _bookingService.getLastBookingCode().done(function (result) {
            if (result == null) {
                var one = "B0000001";
                $("#bookingNumber").val(one);
            }
            lastBookingCode = result.bookingCode;
            var getPart = lastBookingCode.replace(/[^\d.]/g, ''); //get number part
            var num = parseInt(getPart); //convert to int
            var newval = num + 1; //add 1
            var reg = new RegExp(num); //dynamic regexp
            newBookingCode = lastBookingCode.replace(reg, newval);
            $("#bookingNumber").val(newBookingCode);
        });
        for (var i = 0; i < 10; i++) {
            $("#bookingTable").append(
                "<tr id='rowProd" + i + "'>" +
        ...
        _$form.find('button[type="submit"]').click(function (e) {
            ...
            abp.ajax({
                url: abp.appPath + 'Booking/CreateBooking',
                type: 'POST',
            ...
            _bookingService.create(booking).done(function () {
                location.href = "/Booking";
            }).always(function () {
