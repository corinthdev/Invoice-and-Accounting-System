(function () {
    $(function () {
        ...
        var _$form = $('form[name=debitMemoCreateForm]');
        ...
        var caseprice;
        _$form.validate({});
        var lastDebitMemoCode;
        var newDebitMemoCode;

        let today = new Date().toLocaleDateString();
        $("#debitMemoDate").val(today);
        ...
        _debitMemoService.getLastDebitMemoCode().done(function (result) {
            if (result == null) {
                var one = "DM0000001";
                $("#debitMemoNumber").val(one);
            }
            lastDebitMemoCode = result.debitMemoCode;
            var getPart = lastDebitMemoCode.replace(/[^\d.]/g, ''); //get number part
            var num = parseInt(getPart); //convert to int
            var newval = num + 1; //add 1
            var reg = new RegExp(num); //dynamic regexp
            newDebitMemoCode = lastDebitMemoCode.replace(reg, newval);
            $("#debitMemoNumber").val(newDebitMemoCode);
        });
        for (var i = 0; i < 10; i++) {
        ...
        _$form.find('button[type="submit"]').click(function (e) {
            ...
            abp.ajax({
                url: abp.appPath + 'DebitMemo/CreateDebitMemo',
                type: 'POST',
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                dataType: 'json',
                data: debitMemo,
                success: function () {
                    location.href = "/DebitMemo";
                },
                error: function (e) { }
            });

            /*abp.ui.setBusy(_$form);
            _debitMemoService.create(debitMemo).done(function () {
                location.href = "/DebitMemo";
            }).always(function () {
